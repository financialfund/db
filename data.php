...
<script type="text/javascript">
    ...
    $query = mysqli_query($con, "select * from growth");
    for ($i=0; $i < mysqli_num_rows($query); $i++) { 
        $re = mysqli_fetch_array($query);

            ?>
                myObj["date"].push(<?php echo "'",strval($re["date"]),"'";?>);
                incr.push(<?php echo $re["growth"];?>);
            <?php
        }
    ...
    $query = mysqli_query($con, "select * from stat");
    $re = mysqli_fetch_array($query);

    ?>

    var out = <?php echo $re["outstanding"]; ?>;
    var management = <?php echo $re["aum"]; ?>;
    var insider = <?php echo $re["insider"]; ?>;
